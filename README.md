...
- Usage polling is adaptive: the active account stays current, idle alternates normally back off to 30–60 minutes, and a selected target is revalidated before switching. Exhausted accounts are left alone until they reset.
- It fails safe: an HTTP 429 on the active account triggers immediate failover to a freshly verified candidate instead of trusting stale usage. Other transient errors use the normal unhealthy-poll debounce, and an expired token on an idle machine makes it hold rather than fail over (Claude Code refreshes the token on your next message).
- An account whose refresh token has died is quarantined and reported until you either log in with it and re-run `cswap add --slot N`, or replace its stored credentials from a known-good export with `cswap import backup.cswap --force`. API-key accounts are never rotated onto unless you pass `--include-api-key-accounts`.
...
Shows every account's 5h / 7d / configured-model / spend usage, reset countdowns, stale-data age, and disabled state, and switches with a click (specific / rotate / best / next-available), plus add / disable-enable / remove / refresh actions. Choose *Settings → Title percentage → 5h · 7d · configured model* to include Fable in the menu-bar title. Enable *Settings → Auto-switch accounts* to run the same engine as [`cswap auto`](#automatic-switching) in the background; it shares the `autoswitch.*` settings, so the menu bar and CLI stay in sync. Off until you turn it on.

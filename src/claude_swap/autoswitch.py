...
class PollEvent(AutoSwitchEvent):
    ...
    def human(self) -> str:
        ...
        return (
            ...
            f"(switch at {pct_label(boundary)}%){tail}"
        )
...
class AutoSwitchEngine:
                ...
                if not safe_burn:
                    self._emit(
                        NoSwitchEvent(
                            ...
                            detail=(
                                f"{active_gate.window + ' ' if safe_burn and active_gate else ''}"
                                f"{pct_label(utilization)}% < "
                                f"{pct_label(switch_threshold)}%"
                            ),
                        )
                    )
                    return TickOutcome.NO_ACTION
    ...
    def _collect_scheduled_usage(
        ...
        *,
        ...
        Candidate selection never runs on the pre-escalation snapshot, and
        safe-burn revalidates its chosen target immediately before switching.

...
SHARED_CREDENTIALS_PREV_USERNAME = "shared-oauth.prev"
...
class CredentialStore:
    ...
    def _shared_prev_path(self) -> Path:
        return self._host.credentials_dir / ".shared-oauth.enc.prev"
    ...
    def _read_shared_credentials(self) -> str | None:
        ...
        read safely. Only *environmental* failures (an unreadable file, a locked
        Keychain) report ``None``: they may mask a real snapshot, so activation
        must fail closed. Deterministically corrupt file *content* is treated as
        uninitialized instead — failing closed on it would brick every
        API-key→OAuth switch with no code path that ever rewrites the file.
        """
        ...
        if enc_file.exists():
            try:
                ...
                self._host._logger.warning(
                    f"Failed to read shared credentials file {enc_file}: {e}"
                )
                file_failed = True
            else:
                try:
                    ...
                    self._host._logger.warning(
                        f"Corrupt shared credentials file {enc_file} ({e}); "
                        "treating the snapshot as uninitialized"
                    )
                    decoded = ""
                if decoded:
                    return decoded
        ...
        if self._host.platform == Platform.MACOS:
            # Always attempt the read directly, bypassing the capability
            # cache: this separately named snapshot is read-only here, a
            # cooldown armed by an unrelated op must not abort an activation,
            # and under a pinned file mode (which protects *active*
            # credentials from a stale Keychain item) this snapshot is still
            # authoritative. The direct call changes no routing and schedules
            # no re-probe.
            try:
                credentials = macos_keychain.get_password(
    ...
    def _retain_previous_shared(self, new_credentials: str) -> None:
        """Retain the current shared snapshot as ``.prev`` before replacing it.

        The snapshot can hold the only copy of every MCP refresh token (the
        live blob may have been rewritten without siblings by a fresh Claude
        login), so mirror the account-backup retention: one best-effort
        generation, Keychain when the Keychain is in use, ``.enc.prev`` file
        otherwise — never weakening the storage posture just for recovery.
        """
        current = self._read_shared_credentials()
        ...
        try:
            if self._use_keychain():
                ...
                self._atomic_b64_write(self._shared_prev_path(), current)
        except Exception as e:
            self._host._logger.warning(
                f"Failed to retain previous shared OAuth snapshot: {e}"
            )

    def _write_shared_credentials(self, credentials: str) -> None:
        """Persist shared credential fields independently of account slots."""
        self._retain_previous_shared(credentials)
        if self._use_keychain():
            try:
                self._kc_call(
                    macos_keychain.set_password,
        ...
        if self._host.platform == Platform.MACOS:
            try:
                ...
                self._host._logger.warning(
                    "Could not delete the stale shared Keychain item after a "
                    f"file-fallback write: {e}"
                )

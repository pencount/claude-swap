...
class TestReservedAccounts:
    def _seed(self, temp_home: Path, **settings) -> EngineHarness:
        h = EngineHarness(temp_home, **settings)
        h.seed(1, "a@example.com")
        h.seed(2, "desktop@example.com")
        h.seed(3, "c@example.com")
        h.make_live("a@example.com", 1)
        return h

    def test_best_strategy_skips_reserved_email_but_keeps_usage_visible(self, temp_home):
        h = self._seed(
            temp_home,
            reserved_accounts="DESKTOP@example.com",
        )
        outcome = h.tick_with_usage({
            "1": _usage(95),
            "2": _usage(10),
        ...
        assert h.active_number() == 3
        poll = next(event for event in h.events if isinstance(event, PollEvent))
        assert poll.headroom["2"] == 90.0

    def test_safe_burn_skips_reserved_earliest_deadline(self, temp_home):
        h = self._seed(
            temp_home,
            strategy="safe-burn",
            model="Fable",
        ...
        outcome = h.tick_with_usage({
            "1": _model_usage_with_reset(
                5, 5, 20, fable_reset="2030-01-03T00:00:00Z"
            ),
            "2": _model_usage_with_reset(
                5, 5, 10, fable_reset="2030-01-01T00:00:00Z"
            ),
            "3": _model_usage_with_reset(
                5, 5, 30, fable_reset="2030-01-02T00:00:00Z"
            ),
        })
        assert outcome is TickOutcome.SWITCHED
        assert h.active_number() == 3

    def test_slot_number_reservation_blocks_all_automatic_targets(self, temp_home):
        h = self._seed(temp_home, reserved_accounts="2,3")
        outcome = h.tick_with_usage({
            "1": _usage(100),
            "2": _usage(10),
            "3": _usage(20),
        ...
        assert outcome is TickOutcome.BLOCKED
        assert h.active_number() == 1
        event = next(event for event in h.events if isinstance(event, NoSwitchEvent))
        assert event.reason == "no-candidates"
        assert "reserved" in event.detail

    def test_manual_switch_remains_an_explicit_override(self, temp_home):
        h = self._seed(temp_home, reserved_accounts="desktop@example.com")

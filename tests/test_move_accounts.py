...
class TestMoveAccount:
    ...
    def test_move_failed_required_clear_aborts_commit(
        ...
        with monkeypatch.context() as injected:
            injected.setattr(Path, "unlink", failing_unlink)
    ...
    def test_move_strict_clear_fails_closed_on_unreadable_dir(
        ...
        switcher = ClaudeAccountSwitcher()
        # Exercise the file backend even on macOS: this test is specifically
        # about an unreadable credentials directory, not Keychain deletion.
    ...
    def test_move_strict_clear_fails_closed_on_locked_keychain(
        ...
        with monkeypatch.context() as injected:
            injected.setattr(macos_keychain, "get_password", locked)
            injected.setattr(macos_keychain, "delete_password", locked)
            with pytest.raises(CredentialError, match="aborting before commit"):
                switcher.move_account("2", "5")
